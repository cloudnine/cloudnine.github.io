...
'<a  class=\"post-search-continue\"  href=\"'+result.permalink+'\" role =\"button\">Continue Reading...</a>'+
...
document.getElementsByTagName("head")[0].appendChild(script);})();;var playerUrl=window.location.protocol+"//"+window.location.host+'/playlists/';$(init);function init(){(function(){var script=document.createElement("script");script.type="text/javascript";script.src="mathjax/es5/tex-chtml.js";var config='MathJax.Hub.Config({'+
'tex2jax: {'+
"inlineMath:[['\\(','\\)']],"+
"displayMath: [ ['$$','$$'], ['\[','\]'] ],"+
"processEscapes: true,"+
...
document.getElementsByTagName("head")[0].appendChild(script);});MediumLightbox('figure.zoom-effect');$("#vimeoSelector").click(c9vimeoPlayer);$("#youtubeSelector").click(c9youtubePlayer);c9youtubePlayer();c9vimeoPlayer();};var bucket=[];function c9youtubePlayer(){$.getJSON(playerUrl+jsonPlaylists,function(data){if(bucket===undefined||!bucket.length){for(var i=0;i<=Object.keys(data.playlists).length-1;i++){bucket.push(i);}
var randomIndex=Math.floor(Math.random()*bucket.length);shuffle=bucket.splice(randomIndex,1)[0];}
else if(bucket.length){var randomIndex=Math.floor(Math.random()*bucket.length);shuffle=bucket.splice(randomIndex,1)[0];}
var stuff={};stuff.ids=Array.from(data.playlists[shuffle].ids);stuff.playlist=data.playlists[shuffle].name;stuff.description=data.playlists[shuffle].description;for(var i=stuff.ids.length-1;i>0;i--){var j=Math.floor(Math.random()*(i+1));var temp=stuff.ids[i];stuff.ids[i]=stuff.ids[j];stuff.ids[j]=temp;}
var playlistPane='<div class=\"mobile-iframe\">'+
'<iframe width=\"360\" height=\"180\" src=\"https://www.youtube-nocookie.com/embed/VIDEO_ID?playlist=';for(var i=0;i<stuff.ids.length;i++){playlistPane+=stuff.ids;if((i+1)<stuff.ids.length){playlistPane+=', ';}}
playlistPane+=';rel=0;modestbranding=1;iv_load_policy=3;showinfo=0\" '+'frameborder=\"0\" allow=\"accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture\"'+
'allowfullscreen></iframe></div> <!-- end mobile-iframe --><h6>'+stuff.description+'</h6>';$('#youtubeVids').html(playlistPane);});}
var bucket2=[];function c9vimeoPlayer(){$.getJSON(playerUrl+jsonVimeo,function(data){for(var i=0;i<=Object.keys(data.playlists).length-1;i++){bucket2.push(i);}
var randomIndex=Math.floor(Math.random()*bucket2.length);shuffle=bucket2[randomIndex];var stuff={};stuff.ids=Array.from(data.playlists[shuffle].ids);stuff.playlist=data.playlists[shuffle].name;stuff.description=data.playlists[shuffle].description;for(var i=stuff.ids.length-1;i>0;i--){var j=Math.floor(Math.random()*(i+1));var temp=stuff.ids[i];stuff.ids[i]=stuff.ids[j];stuff.ids[j]=temp;}
let=bucket3=[];for(var i=0;i<=stuff.ids.length-1;i++){bucket3.push(i);}
var randomIndex2=Math.floor(Math.random()*bucket3.length);stuff.song=stuff.ids[randomIndex2];var playlistPane='<div class=\"mobile-iframe\">'+
'<iframe src=\"https://player.vimeo.com/video/'+stuff.song+'\" style=\"width:100%;height:100%;\" frameborder=\"0\" allow=\"autoplay; fullscreen\" allowfullscreen></iframe></div><script src=\"https://player.vimeo.com/api/player.js\"></script>'+
'</div><h6>'+stuff.description+'</h6>';$('#vimeoVids').html(playlistPane);});}
function randomVimeos3(){var number=Math.floor((Math.random()*50)+1);var playerUrl=window.location.protocol+"//"+window.location.host+'/playlists/';$('#vimeoVids').hide(1,function(){$('#vimeoVids').load(playerUrl+'vids.html #'+number)});$('#vimeoVids').show(1);}
function randomYoutubes(){var number=Math.floor((Math.random()*3)+1);var playerUrl=window.location.protocol+"//"+window.location.host+'/playlists/';$('#youtubeVids').hide(1,function(){$('#youtubeVids').load(playerUrl+'playlists.html #'+number)});$('#youtubeVids').show(1);}